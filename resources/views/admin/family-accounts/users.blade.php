...
{{--             @include('admin.family-accounts.user-detail')--}}
        </div>
    </aside>
    <!--************************************
            Sidebar End
                                    ...
                                    <div class="dropdown at-groupdropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                            ...
                                            @if(auth()->user()->isAdmin())
                                                <a class="dropdown-item" href="{{route('users.group.create',['id'=>$family_account->id])}}">view group</a>
                                            @elseif(auth()->user()->isFamilyAccountant())
                                                <a class="dropdown-item" href="{{route('family.admin.users.group.create',['id'=>$family_account->id])}}">view group</a>
                                            @endif
                                        </div>
                                    </div>
                                </li>
    ...
    *************************************-->
    <div class="modal fade at-modaltheme at-choosedisplaymodal" id="exampleModalCentervthree" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    <form class="at-modalform at-formtheme" action="{{route('family.user.filters')}}" method="post">
                        @csrf
                        <fieldset>
                            <div class="at-modaltitle">
                    ...
                    @endif
                        ...
                        <fieldset>
                            ...
                            <ul class="at-reportusers">
                                @php( $group_users = family_account($family_account->id))
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click','.at-user',function(e){
                ...
                let name = $(this).find('.first-name').text();
                $('.name').text(name);
                $('.at-userimage img').attr('src',image);

                let id = $(this).data('id');
                console.log(id);
                $.get('/family/user/detail/'+id,function (response) {
            ...
            $(document).on('click','#at-checkboxfive', function (e) {
                ...
                $.post('/family/user/assign/questionnaire/',data,function (response) {
